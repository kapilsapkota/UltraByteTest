        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.pages.update', $data['page']->id) }}">
                    @csrf
                    @method('PUT')
                    <!-- Name -->
                        <div>
                            <label for="newstitle" class="block font-medium text-sm text-gray-700">Title <strong>*</strong></label>
                            <input type="text" name="title" value="{{ $data['page']->title }}" placeholder="Enter post title." class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" id="newstitle">
                            @if($errors -> has('title'))
                        ...
                        <div class="mt-4">
                            ...
                            <input type="file" accept="image/*" name="main_image" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @endif
                            <img src="{{asset('images/'.$data['page']->image)}}" alt="{{$data['page']->title}}">
                        </div>
                        <div class="mt-4">
